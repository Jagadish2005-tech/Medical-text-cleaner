...
def generate_line_chart(word_counts, title, file_path):
    word_counts = word_counts.sort_values(ascending=False)[:10]
    plt.figure(figsize=(10, 6))
    plt.plot(word_counts.index, word_counts.values, marker='o', color='green', linestyle='-')
    plt.title(title)
    plt.xlabel('Words')
    plt.ylabel('Frequency')
    plt.xticks(rotation=45)
...
def generate_pie_chart(word_counts, title, file_path):
    word_counts = word_counts.sort_values(ascending=False)[:5]
    plt.figure(figsize=(8, 8))
    plt.pie(word_counts.values, labels=word_counts.index, autopct='%1.1f%%', startangle=140)
    plt.title(title)
    plt.tight_layout()
    plt.savefig(file_path)
    plt.close()
...
for filename in os.listdir(input_dir):
    ...
    generate_bar_chart(cleaned_counts, f"Top Words (Cleaned) - {filename}", os.path.join(visual_dir, output_base + '_cleaned_bar.png'))
    generate_line_chart(cleaned_counts, f"Top Words Line Chart (Cleaned) - {filename}", os.path.join(visual_dir, output_base + '_cleaned_line.png'))
    generate_pie_chart(cleaned_counts, f"Top 5 Word Share (Cleaned) - {filename}", os.path.join(visual_dir, output_base + '_cleaned_pie.png'))
